    ...
    <section className="panel panel-crown p-5 lg:p-6 animate-fade-up">
      ...
      <ul className="mt-4 grid gap-3">
        {insights.map((item) => (
          <li
            key={item.id}
            className={cn("rounded-xl border px-3.5 py-3", toneClass[item.tone])}
          >
            <div className="flex items-start gap-2.5">
              <span className={cn("mt-1.5 size-2 shrink-0 rounded-full", toneDot[item.tone])} />
              <div className="min-w-0">
                <p className="text-sm font-semibold">{item.title}</p>
                <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">{item.body}</p>
                {item.tip ? (
                  <p className="mt-2 flex items-start gap-1.5 text-xs font-medium text-primary-dark">
