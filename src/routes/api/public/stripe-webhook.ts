  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const applyPlan = async (userId: string | null, customerId: string | null, plan: PlanId, status: string, subscriptionId: string | null) => {
          const patch = { plan, plan_status: status, stripe_subscription_id: subscriptionId, ...(customerId ? { stripe_customer_id: customerId } : {}) };
          if (userId) await supabaseAdmin.from("profiles").update(patch).eq("id", userId);
          else if (customerId) await supabaseAdmin.from("profiles").update(patch).eq("stripe_customer_id", customerId);
        };
        ...
        const asId = (v: unknown): string | null => typeof v === "string" ? v : ((v as { id?: string } | null)?.id ?? null);

        if (event.type === "checkout.session.completed") {
          const session = event.data.object;
          const customerId = asId(session.customer);
          const subscriptionId = asId(session.subscription);
          const userId = await resolveUserId(session.metadata?.["user_id"] ?? session.client_reference_id, customerId);
        ...
        if (event.type === "customer.subscription.updated") {
          ...
          const paid = ["active", "trialing"].includes(sub.status);
          const plan = paid ? (planFromPrice(sub.items.data[0]?.price.id) ?? "free") : "free";
          await applyPlan(userId, customerId, plan, sub.status, paid ? sub.id : null);
          return new Response("ok");
        }
